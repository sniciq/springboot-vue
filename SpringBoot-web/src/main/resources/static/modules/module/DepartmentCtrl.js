angular.module('myApp').controller('DepartmentCtrl', function($scope, $uibModal, $http, $anchorScroll, confirmDialogs){
	...
	$scope.search = function() {
		...
		$scope.promise = $http.post(
			ctx + '/department/DepartmentCtrl/search', 
	...
	$scope.edit = function(item) {
		...
		$uibModal.open({
			templateUrl: ctx + '/modules/module/DepartmentEditTpl.html?v=',
	...
	$scope.del = function(item) {
		...
		.result.then(function (btn) {
			if(btn == 'ok') {
				$scope.promise = $http.get(ctx + '/department/DepartmentCtrl/delete?id='+item.id).success(function(data){
...
angular.module('myApp').controller('DepartmentEditCtrl', function($scope, $http, $uibModalInstance, confirmDialogs,id){
	...
	if(id && id > 0) {
		$scope.title = '编辑';
		$scope.promise = $http.get(ctx + '/department/DepartmentCtrl/getDetailInfo?id='+id).success(function(data){
	...
	$scope.save = function() {
		$scope.promise = $http.post(ctx + '/department/DepartmentCtrl/save', $scope.editForm).success(function(data){
