angular.module('myApp').controller('StaffCtrl', function($scope, $uibModal, $http, $anchorScroll, confirmDialogs){
	...
	$scope.edit = function(item) {
		...
		$uibModal.open({
			templateUrl: ctx + '/modules/module/StaffEditTpl.html?v=',
	...
	$scope.del = function(item) {
		...
		.result.then(function (btn) {
			if(btn == 'ok') {
				$scope.promise = $http.get(ctx + '/staff/StaffCtrl/delete?id='+item.id).success(function(data){
...
angular.module('myApp').controller('StaffEditCtrl', function($scope, $http, $uibModalInstance, confirmDialogs,id){
	...
	if(id && id > 0) {
		$scope.title = '编辑';
		$scope.promise = $http.get(ctx + '/staff/StaffCtrl/getDetailInfo?id='+id).success(function(data){
	...
	$scope.save = function() {
		$scope.promise = $http.post(ctx + '/staff/StaffCtrl/save', $scope.editForm).success(function(data){
